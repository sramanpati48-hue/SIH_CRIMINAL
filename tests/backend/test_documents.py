...
class TestCreateDocument:
    ...
    def test_create_document_success(self, client):
        ...
        response = client.post(f"/api/v1/cases/{case_id}/documents", json=payload)
    ...
    def test_create_document_case_not_found(self, client):
        """Upload to non-existent case returns 404."""
        payload = {
            "file_name": "orphan.csv",
            "file_type": "CDR",
        }
        response = client.post("/api/v1/cases/non-existent-uuid/documents", json=payload)
...
class TestListDocuments:
    ...
    def test_list_documents_returns_uploaded(self, client):
        ...
        client.post(f"/api/v1/cases/{case_id}/documents", json={
    ...
    def test_list_documents_case_not_found(self, client):
        """Listing documents for non-existent case returns 404."""
        response = client.get("/api/v1/cases/non-existent-uuid/documents")
        assert response.status_code == 404
...
class TestDocumentAuditLogging:
    ...
    def test_upload_document_generates_audit_log(self, client, db_session):
