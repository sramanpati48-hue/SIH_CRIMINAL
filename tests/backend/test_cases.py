...
class TestCreateCase:
    ...
    def test_create_case_success(self, admin_client):
        ...
        response = admin_client.post("/api/v1/cases", json=payload)
    ...
    def test_create_case_default_priority(self, admin_client):
    ...
    def test_create_case_duplicate_number_rejected(self, admin_client):
        """Duplicate case number returns 409 Conflict."""
        payload = {
            "case_number": "CASE-DUP-001",
            "title": "First Case",
        }
        response1 = admin_client.post("/api/v1/cases", json=payload)
...
class TestListCases:
    ...
    def test_list_cases_empty(self, admin_client):
        ...
        response = admin_client.get("/api/v1/cases")
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 0
        assert data["cases"] == []

    def test_list_cases_returns_created(self, admin_client):
        """Created cases appear in the list."""
        admin_client.post("/api/v1/cases", json={
    ...
    def test_list_cases_status_filter(self, admin_client):
        """Status filter returns only matching cases."""
        admin_client.post("/api/v1/cases", json={
            "case_number": "CASE-FILTER-001",
            "title": "Active Case",
        })
        response = admin_client.get("/api/v1/cases?status=CLOSED")
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 0
...
class TestGetCase:
    ...
    def test_get_case_success(self, admin_client):
        """Retrieve an existing case by ID."""
        create_resp = admin_client.post("/api/v1/cases", json={
            "case_number": "CASE-GET-001",
            "title": "Get Test Case",
        })
        case_id = create_resp.json()["id"]

        response = admin_client.get(f"/api/v1/cases/{case_id}")
        assert response.status_code == 200
    ...
    def test_get_case_not_found(self, admin_client):
        ...
        response = admin_client.get("/api/v1/cases/non-existent-uuid")
...
class TestUpdateCase:
    ...
    def test_update_case_title(self, admin_client):
        ...
        response = admin_client.patch(f"/api/v1/cases/{case_id}", json={
            "title": "Updated Title",
        })
        assert response.status_code == 200
    ...
    def test_update_case_status(self, admin_client):
    ...
    def test_update_case_not_found(self, admin_client):
        ...
        response = admin_client.patch("/api/v1/cases/non-existent-uuid", json={
            "title": "No such case",
        })
        assert response.status_code == 404
...
class TestAuditLogging:
    ...
    def test_create_case_generates_audit_log(self, admin_client, db_session):
    ...
    def test_update_case_generates_audit_log(self, admin_client, db_session):
