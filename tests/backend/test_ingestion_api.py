...
def test_upload_invalid_file_extension_2(client):
    with open(__file__, 'rb') as f:
        response = client.post(
            "/api/v1/cases/C001/upload",
            files={"file": ("test_ingestion_api.py", f, "text/x-python")}
...
def test_ingest_synthetic_endpoint(mock_ingest, mock_exists, client):
    ...
    response = client.post("/api/v1/cases/C001/ingest-synthetic")
    assert response.status_code == 200
    assert "Synthetic data ingested" in response.json()["message"]

def test_get_ingestion_summary(client, db_session):
    # Setup mock job
    job = ProcessingJob(case_id="C001", job_type="TEST", status="COMPLETED", total_rows=5)
    db_session.add(job)
    db_session.commit()
    
    response = client.get("/api/v1/cases/C001/ingestion-summary")
    assert response.status_code == 200
    data = response.json()
...
def test_process_document(client):
    response = client.post("/api/v1/documents/doc1/process")
    assert response.status_code == 200

def test_retry_graph_sync(client):
    response = client.post("/api/v1/graph/sync/retry?case_id=C001")
    assert response.status_code == 200
